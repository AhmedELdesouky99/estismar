...
import { admin, serviceProvider } from "../../util/axios";
      ...
      serviceProvider
        .get(`/service`, {
          params: {
            ...
            support_ratio: query.support_ratio
              ? query.support_ratio
              : undefined,
          },
        })
      ...
      admin
        .get(`/service`, {
          params: {
            ...
            service_provider_id:
              id && location.pathname.includes("service-provider")
                ? id
                : user?.category == "service-provider" ||
                  user?.category == "provider-employee"
                ? user?.id
                : query.service_provider_id
                ? query.service_provider_id
                : undefined,
            status: query.status ? query.status : undefined,
            support_ratio: query.support_ratio
            ...
            <div className='row'>
              <FiltersAndSearches
                ...
                filters={
                  id
                    ? ["parent", "status", "fields", "support"]
                    : ["parent", "status", "fields", "service_provider"]
                }
                query={query}
                setPage={setPage}
                setQuery={setQuery}
