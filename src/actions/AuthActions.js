...
export const HandelSignin = (user, history) => (dispatch) => {
  axios
    .post("https://admin.waqfnami.com/api/auth/login", { ...user })
    .then((res) => {
      if (res.status == 200) {
    ...
    .catch((res) => {
      dispatch({
        ...
        payload: "البريد الالكتروني او كلمة المرور غير صحيح ",
      });
    });
};
export const HandelSignUp = (user, history) => (dispatch) => {
  axios
    .post("https://admin.waqfnami.com/api/auth/register", { ...user })
    .then((res) => {
      if (!res.data.errors) {
        localStorage.setItem("user_id", JSON.stringify(res?.data?.data));
        localStorage.setItem("token", res.data?.data?.access_token);
